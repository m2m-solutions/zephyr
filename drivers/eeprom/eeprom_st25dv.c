				...
				uint32_t count)
{
	...
	if((ret = i2c_write_read(self->i2c, self->config->addr, wr_data, 2, pdata,
				    count)) < 0){
				...
				uint32_t count)
{
	uint8_t wr_data[2] = {addr >> 8, addr & 0xff};
	int ret = 0;
	if((ret = i2c_write_read(self->i2c, self->config->addr | 4, wr_data, 2, pdata, count)) < 0){
...
static int eeprom_st25dv_read(const struct device *dev, off_t offset,
				void *buf,
				size_t len)
{
...
static int eeprom_st25dv_write(const struct device *dev, off_t offset,
...
static int eeprom_st25dv_init(const struct device *dev)
{
	...
	LOG_INF("UUID: %02x%02x%02x%02x%02x%02x%02x%02x",
					uuid[7],
					uuid[6],
					uuid[5],
					uuid[4],
...
#if CONFIG_PM
static int eeprom_st25dv_pm(const struct device *dev,
													 uint32_t ctrl_cmd,
													 uint32_t *new_state,
													 pm_device_cb cb,
													 void *arg){
